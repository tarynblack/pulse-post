  ...
  % =================== B E G I N   T I M E   L O O P =================== %
    ...
    while t <= timesteps 
      ...
      % Save density and buoyancy calculations at each timestep
        dlmwrite(fullfile(sprintf('%s',dir),sprintf('flowdensity_t%d.txt',...
            time(t))),flowdensity,'delimiter','\t','precision','%g');
        dlmwrite(fullfile(sprintf('%s',dir),sprintf('atmsdensity_t%d.txt',...
            time(t))),atmsdensity,'delimiter','\t','precision','%g');
        dlmwrite(fullfile(sprintf('%s',dir),sprintf('flowbuoyancy_t%d.txt',...
